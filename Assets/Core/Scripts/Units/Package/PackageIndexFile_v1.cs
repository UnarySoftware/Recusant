...
namespace Core
{
    public class PackageIndexFile_v1 : PackageIndexFile
    {
        ...
        public override bool Read(string modName, string file)
        {
            ...
            for (uint i = 0; i < dirEntryCount; i++)
            {
                ...
                EntryList.Add(new()
                {
                    ...
                    Path = path,
                    CapitalizedLength = capitalizedLength,
                    Capitalized = capitalized,
                    Type = ContentLoader.GetAssetFileType(path),
                    Archive = archive,
                    Offset = offset,
                    Size = size,
                    Crc = crc,
        ...
        public override bool Write(string file)
        {
            ...
            {
                binaryWriter.Write(entry.PathLength);
                binaryWriter.Write(Encoding.ASCII.GetBytes(entry.Path));
                binaryWriter.Write(entry.CapitalizedLength);
                if (entry.CapitalizedLength > 0)
                {
                    binaryWriter.Write(Encoding.ASCII.GetBytes(entry.Capitalized));
                }
