...
public class GameplayExecutorShared : CoreSystemShared
{
...
public class GameplayExecutor : CoreSystemNetwork<GameplayExecutor, GameplayExecutorShared>
{
    ...
    public void SpawnCubeCmd(Vector3 coords)
    {
        ...
        Logger.Instance.Log("Spawning cube at " + GameplayShared.StringifyVariable(coords, GameplayType.Vector3));
    }
    ...
    public void Execute(string line)
    {
        if (!line.Contains(" "))
        {
            Logger.Instance.Error("Invalid variable/command format: " + line);
            return;
        }
        ...
        {
            Logger.Instance.Error("Invalid variable/command: " + key);
            return;
        }
        ...
        if (GameplayExecutorNetwork.Instance != null && GameplayExecutorNetwork.Instance.IsClient)
        {
            if (unit.IsVariable)
            {
                if (unit.Variable.GetGroup() == GameplayGroup.Server)
                {
                    ...
                    {
                        Logger.Instance.Error("Cant change \"" + key + "\" variable since it is in a Server group but cant be replicated");
                        return;
                    }
                }
            }
            else
            {
                if (unit.Command.Command.Group == GameplayGroup.Server)
                {
                    ...
                    {
                        Logger.Instance.Error("Cant call \"" + key + "\" command since it is in a Server group but cant be replicated");
                        return;
                    }
                }
            }
        ...
        {
            Logger.Instance.Error("Failed to parse variable/command value: " + e.Message);
            return;
        }
        ...
        else
        {
            ...
            foreach (var argument in arguments)
            {
                ...
                {
                    Logger.Instance.Error("Invalid argument #" + count + " in command \"" + key + "\"");
                    valid = false;
                }
                count++;
            }
            ...
            {
                Logger.Instance.Error("Failed parsing arguments for command \"" + key + "\"");
                return;
            }
            ...
            {
                Logger.Instance.Error("Argument count mismatch for command \"" + key + "\"");
                return;
            }
            ...
            CoreSystemBase targetSystem = Core.Instance.GetSystem(unit.Command.SystemType);

            if (targetSystem == null)
            {
                Logger.Instance.Error("Failed to find system of type \"" + unit.Command.SystemType.Name + "\" for command \"" + key + "\"");
                return;
            }
    ...
    public void RecieveResult(NetworkArrayStruct4<Logger.Type> types, NetworkArrayStruct4<NetworkString128> lines, NetworkArrayStruct4<NetworkString256> stackTraces)
    {
        for (int i = 0; i < types.Length; i++)
        {
            Logger.Type type = types[i];
            string line = lines[i];
            string stackTrace = stackTraces[i];

            if (type == Logger.Type.None || string.IsNullOrEmpty(line) || string.IsNullOrWhiteSpace(line))
            {
                continue;
            }
            ...
            switch (type)
            {
                ...
                case Logger.Type.Warning:
                    {
                        Logger.Instance.SetCustomTrace(stackTrace);
                        Logger.Instance.Warning(line);
                        break;
                    }
    ...
    {
        Type[] types = Core.GetTypeStatic().Assembly.GetTypes();
        ...
        foreach (Type type in types)
        {
            ...
            if (!typeof(CoreSystemBase).IsAssignableFrom(type))
            {
                continue;
            }
