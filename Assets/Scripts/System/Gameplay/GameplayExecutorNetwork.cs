...
public class GameplayExecutorNetwork : CoreSystemPrefab<GameplayExecutorNetwork, GameplayExecutorShared>
{
    ...
    public void SendCmd(NetworkString128 line)
    {
        Logger.Instance.StartCollecting();

        GameplayExecutor.Instance.Execute(line);

        var connection = Sandbox.CurrentRpcSource;

        NetworkArrayStruct4<Logger.Type> types = new();
        ...
        RpcRelay relay = RpcDispatcher.Instance.GetRelay(connection.PlayerId);
